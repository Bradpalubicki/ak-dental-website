...
export function BillingClient({ data, analytics }: { data: BillingData; analytics: BillingAnalytics }) {
  ...
  const [appealClaim, setAppealClaim] = useState<BillingClaim | null>(null);
  ...
  const openAppeal = async (claim: BillingClaim) => {
    setAppealClaim(claim);
    setAppealLetter("");
    setAppealCopied(false);
    setGeneratingAppeal(true);
    try {
      const res = await fetch("/api/billing/appeal", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ claim }),
      });
      const json = await res.json() as { letter?: string; error?: string };
      setAppealLetter(json.letter || "Unable to generate letter. Please try again.");
    } catch {
      setAppealLetter("Unable to generate letter. Please try again.");
    } finally {
                          ...
                          <td className="px-6 py-3 text-center">
                            {needsAction ? (
                              <button
                                ...
                                className="inline-flex items-center gap-1 rounded-md bg-red-50 border border-red-200 px-2 py-1 text-[10px] font-semibold text-red-700 hover:bg-red-100 transition-colors"
                              >
          ...
          <div className="w-full max-w-2xl rounded-2xl bg-white shadow-2xl flex flex-col max-h-[90vh]">
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100">
              <div className="flex items-center gap-2">
                ...
                <h2 className="text-sm font-semibold text-slate-900">AI Appeal Letter</h2>
                <span className="text-[10px] text-slate-400 border border-slate-200 rounded px-1.5 py-0.5">
                  {appealClaim.claim_number || "Claim"} · {appealClaim.insurance_provider}
                </span>
              </div>
              <button
                onClick={() => setAppealClaim(null)}
            ...
            {/* Claim Summary */}
            <div className="px-6 py-3 border-b border-slate-100 bg-slate-50 grid grid-cols-3 gap-4 text-center">
              <div>
                <p className="text-[10px] text-slate-400 uppercase tracking-wider">Billed</p>
                <p className="text-sm font-bold text-slate-900">${Number(appealClaim.billed_amount).toLocaleString()}</p>
              </div>
              <div>
                <p className="text-[10px] text-slate-400 uppercase tracking-wider">Denial Reason</p>
                <p className="text-xs font-semibold text-red-700">{appealClaim.denial_reason || "See claim"}</p>
              </div>
              <div>
                <p className="text-[10px] text-slate-400 uppercase tracking-wider">Procedures</p>
                <p className="text-xs font-semibold text-slate-700 truncate">{procedureSummary(appealClaim.procedure_codes)}</p>
              </div>
            </div>
            ...
            <div className="flex-1 overflow-y-auto px-6 py-4">
              {generatingAppeal ? (
                <div className="flex flex-col items-center justify-center py-12 gap-3">
                  ...
                  <p className="text-sm text-slate-500">Drafting appeal letter...</p>
                </div>
              ) : (
                <textarea
                  value={appealLetter}
                  onChange={(e) => setAppealLetter(e.target.value)}
                  className="w-full h-64 rounded-xl border border-slate-200 p-4 text-xs text-slate-700 font-mono leading-relaxed focus:border-cyan-300 focus:ring-1 focus:ring-cyan-200 outline-none resize-none"
            ...
            <div className="flex items-center justify-between px-6 py-4 border-t border-slate-100">
              <p className="text-[10px] text-slate-400">Review and edit before sending. This letter uses procedure codes only — no PHI.</p>
              <div className="flex items-center gap-2">
                <button
                  onClick={() => setAppealClaim(null)}
                ...
                <button
                  onClick={copyAppeal}
                  disabled={!appealLetter || generatingAppeal}
                  className="flex items-center gap-1.5 rounded-lg bg-cyan-600 px-4 py-1.5 text-xs font-semibold text-white hover:bg-cyan-700 disabled:opacity-50 transition-colors"
                >
                  {appealCopied ? <CheckCircle2 className="h-3.5 w-3.5" /> : <Send className="h-3.5 w-3.5" />}
