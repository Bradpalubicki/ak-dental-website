...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const response = await client.disputes.accept({ disputeId: id })
    return NextResponse.json({
      success: true,
      disputeId: id,
      newState: response.dispute?.state ?? 'ACCEPTED',
    })
  } catch {
    return NextResponse.json({ error: 'Failed to accept dispute' }, { status: 500 })
  }
}
